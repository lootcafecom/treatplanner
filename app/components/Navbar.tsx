    ...
    <nav style={{ background:"white", borderBottom:"1px solid var(--border)", position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 12px rgba(0,0,0,0.04)" }}>
      <div className="container" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height:66 }}>
        ...
        <div style={{ display:"flex", gap:28, alignItems:"center" }}>
          {[["How It Works","/how-it-works"],["Procedures","/procedures"],["Calculator","/calculator"],["About","/about"]].map(([label,href])=>(
            <Link key={href} href={href} style={{ textDecoration:"none", color:"var(--slate)", fontWeight:500, fontSize:14, transition:"color 0.2s" }}
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          <Link href="/find" className="btn-primary" style={{ padding:"8px 20px", fontSize:14, background:"var(--teal)" }}>🔍 Find Cheapest Route</Link>
        </div>
      </div>
    </nav>
  );
